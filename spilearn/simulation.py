...
class NeuronCriticSimulation(Simulation):
    ...
    def __init__(self, env, network, scaler, conv, 
                 h_time=50, start_delta=50, 
                 actions_count=2, critics_count=1, 
                 learning_rate=0.01, learning_rate_critic=0.01):
    ...
    def run_state(self, state):
        ...
        self.network.spike_generator.set_input_spikes(
            spike_dict=spike_dict)
        ...
        out_latency = convert_latency([raw_latency], self.actions_count + self.critics_count)
    ...
    def learn_states(self, states, action):
        ...
        spike_dict, \
        full_time = self.network.spike_generator.create_spike_dict(
            dataset=states,
            delta=0.0)
...
#             if running_reward > max_steps - 0.01:
            ...
            for ti in range(1, n_states_max):  # Don't infinite loop while learning
                ...
                self.learning_rate = self.learning_rate_default * delta_t * self.lr_scale * self.alpha_actor
                self.learning_rate_critic = self.learning_rate_default_critic * delta_t * self.lr_scale * self.alpha_critic
